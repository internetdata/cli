...
Examples:
  ...
  $ %[1]s db download bogon_ip_v1 --format mmdb bogon.mmdb
...
func cmdDatabase() error {
	...
	globalFlags()
	apiFlags()
	pflag.StringVar(&fFormat, "format", string(internetdata.FormatCSVGZ), "which published file.")
...
func dbList(ctx context.Context, db *internetdata.DatabaseAPI, asJSON bool) error {
	...
	if len(items) == 0 {
		fmt.Println("no databases listed for this organization")
		return nil
	}
...
//
// Nil without a license: the catalog lists every published database, not just
// the ones an organization holds, so an unlicensed row genuinely has no type.
// The `standing` column already says `unlicensed`, so an empty cell here reads
// correctly rather than needing a word of its own.
func licenseType(d internetdata.Database) string {
...
// licenseTerm renders when a license ends, which is the one thing a holder has
// to act on. An empty string means there is nothing to act on.
func licenseTerm(d internetdata.Database) string {
	switch {
	// Never bought is not the same as lapsed, and the catalog lists both.
	// `standing` already carries the distinction; saying "lapsed" against a
	// database nobody ever held would invent a history.
	case d.Standing == internetdata.StandingUnlicensed:
		return ""
	case d.Standing == internetdata.StandingExpired:
		return "lapsed"
	case d.Expires != nil:
...
func dbChecksum(ctx context.Context, db *internetdata.DatabaseAPI, id, format string, asJSON bool) error {
	sums, err := db.Checksums(ctx, id, internetdata.Format(format))
	if err != nil {
		return explain(err)
	}
	if asJSON {
...
func dbURL(ctx context.Context, db *internetdata.DatabaseAPI, id, format string) error {
	url, err := db.DownloadURL(ctx, id, internetdata.Format(format))
	if err != nil {
		return explain(err)
	}
	fmt.Println(url)
...
func dbDownloads(ctx context.Context, db *internetdata.DatabaseAPI, limit int, asJSON bool) error {
...
func dbDownload(
	ctx context.Context, db *internetdata.DatabaseAPI,
	id, format, out string, toStdout, noVerify bool,
) error {
	f := internetdata.Format(format)
...
func extensionFor(f internetdata.Format) string {
	if f == internetdata.FormatMMDB {
		return "mmdb"
	}
	return "csv.gz"
}
