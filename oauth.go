...
const clientID = "internetdata-cli"
...
func authBaseURL() string {
	...
	return internetdata.DefaultBaseURL
}

// startDeviceAuth asks the authorization server to begin a flow.
func startDeviceAuth(ctx context.Context) (*internetdata.DeviceAuthorization, error) {
	oauth, err := signInAPI()
	if err != nil {
		return nil, err
	}
	dev, err := oauth.DeviceAuthorization(ctx, clientID,
		internetdata.DeviceAuthorizationOptions{Scope: loginScopes})
...
func pollForToken(
	ctx context.Context, dev *internetdata.DeviceAuthorization,
) (*internetdata.TokenResponse, error) {
	oauth, err := signInAPI()
	if err != nil {
		return nil, err
	}
	...
	switch {
	case errors.Is(err, internetdata.ErrOauthAccessDenied):
		return nil, errors.New("the request was denied in the browser")
	case errors.Is(err, internetdata.ErrOauthExpiredToken):
...
func signInAPI() (*internetdata.OauthAPI, error) {
	client, err := internetdata.New(
		internetdata.WithBaseURL(authBaseURL()),
