...
// nothing - and "nothing" is a working state rather than an error, because
// signing in needs no key.
func TestResolveKeyPrecedence(t *testing.T) {
	...
	reset := func() {
		fKey, fSession = "", ""
		os.Unsetenv("INTERNETDATA_API_KEY")
		os.Unsetenv("INTERNETDATA_SESSION")
	}
	t.Cleanup(reset)
	...
	reset()
	t.Setenv("INTERNETDATA_API_KEY", "from-env")
	if key, src := cfg.ResolveKey(); key != "from-env" || src != "INTERNETDATA_API_KEY" {
...
func TestResolveSessionPrecedence(t *testing.T) {
	...
	t.Setenv("INTERNETDATA_SESSION", "b")
