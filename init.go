...
// reason to run without a stored session, not a reason for `internetdata --key
// <key> db list` to stop working. The one command that must not paper over it
// is `login`, which checks again before it writes.
func init() {
	cfg, err := LoadConfig()
	if err != nil {
