...
# manifests/i/InternetData/CLI is absent - so version one is submitted by
...
SCHEMA_VSN="1.12.0"
BRANCH="${ID}-${VSN}"
DIR="manifests/i/InternetData/CLI/${VSN}"
BASE="https://github.com/internetdata/cli/releases/download/v${VSN}"
WORK="$(mktemp -d)"
trap 'rm -rf "$WORK"' EXIT

if gh api "repos/${UPSTREAM}/contents/manifests/i/InternetData/CLI" > /dev/null 2>&1 ; then
