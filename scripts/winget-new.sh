...
# manifests/m/Mslm/InternetData is absent - so version one is submitted by
...
ID="Mslm.InternetData"
FORK="internetdata/winget-pkgs"
UPSTREAM="microsoft/winget-pkgs"
SCHEMA_VSN="1.12.0"
BRANCH="${ID}-${VSN}"
DIR="manifests/m/Mslm/InternetData/${VSN}"
BASE="https://github.com/internetdata/cli/releases/download/v${VSN}"
WORK="$(mktemp -d)"
trap 'rm -rf "$WORK"' EXIT

if gh api "repos/${UPSTREAM}/contents/manifests/m/Mslm/InternetData" > /dev/null 2>&1 ; then
...
function sha_for() {
    local f="internetdata_${VSN}_windows_${1}.zip"
    curl -fsSL -o "${WORK}/${f}" "${BASE}/${f}"
    unzip -l "${WORK}/${f}" | grep -q ' internetdata.exe$' || {
        echo "${f} does not hold internetdata.exe at its root" >&2
        exit 1
    }
...
NestedInstallerFiles:
  - RelativeFilePath: internetdata.exe
    PortableCommandAlias: internetdata
Commands:
  - internetdata
Installers:
  - Architecture: x86
    InstallerUrl: ${BASE}/internetdata_${VSN}_windows_386.zip
    InstallerSha256: ${SHA_X86}
  - Architecture: x64
    InstallerUrl: ${BASE}/internetdata_${VSN}_windows_amd64.zip
    InstallerSha256: ${SHA_X64}
  - Architecture: arm64
    InstallerUrl: ${BASE}/internetdata_${VSN}_windows_arm64.zip
...
License: MIT
LicenseUrl: https://github.com/internetdata/cli/blob/main/LICENSE
ShortDescription: Official CLI for the InternetData API
Description: |-
  List the IP and network databases your organization is licensed for, see what
  is inside one before you fetch it, and download it in CSV.GZ or MMDB, verified
  against its published checksum.

  Needs an API key carrying the db.download scope; internetdata login signs in
  through the browser.
Moniker: internetdata
Tags:
  - ip
  - ip-address
  - asn
...
New package: ${ID} version ${VSN} - the official CLI for the InternetData API (https://internetdata.io). A portable \`internetdata.exe\` shipped inside a zip per architecture (x86, x64, arm64), from https://github.com/internetdata/cli/releases/tag/v${VSN}; the checksums are those of the published zips.
